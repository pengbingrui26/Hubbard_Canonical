...
# sample W and W_sign
def sample_simple(model, psi0, key, batch, params, nthermal, nsample, ninterval):
    taus = params[:-2]
    ...
    sigma = random_init_sigma_vmapped(model.L*2, nlayer, key_init)  # shape: (batch, nlayer, model.L*2)
...
def make_En_and_grad(model, psi0, key, batch, params, nthermal, nsample, ninterval):
    ...
    sigma, W, sign = sample_simple(model, psi0, key, batch, params, nthermal, nsample, ninterval)
...
def sample_fori_loop(model, psi0, key, batch, params, nthermal, nsample, ninterval):
    taus = params[:-2]
    nlayer = int(taus.shape[-1]/2)
    make_W, make_Eloc, make_eloc = init_fn(model)
    ...
    grad_W_sampled = jnp.zeros((nsample, batch, nparam)).astype('complex')
    grad_eloc_sampled = jnp.zeros((nsample, batch, nparam)).astype('complex')
...
def make_En_and_grad_new(model, psi0, key, batch, params, nthermal, nsample, ninterval):
    W, sign, eloc, grad_W, grad_eloc = sample_fori_loop(model, psi0, key, batch, params, nthermal, nsample, ninterval)
...
def make_loss_grad(beta, model, psi0_set, key, batch, params, nthermal, nsample, ninterval):
    ...
    taus_deltas = params[psi0_set.shape[0]:]

    E_all, grad_E_all = make_En_and_grad_new_vmapped(model, psi0_set, key_set, batch, taus_deltas, nthermal, nsample, ninterval)
    ...
    grad_F_taus_deltas = jnp.dot(pp, grad_E_all)
    #print("grad_F_taus_deltas:", grad_F_taus_deltas.shape)

    grad_F = jnp.hstack((grad_F_qq, grad_F_taus_deltas))
...
def optimize_F(beta, model, psi0_set, batch, nthermal, nsample, ninterval, Nlayer):
    ...
    taus_deltas = jnp.array([0.1] * (2*Nlayer+2)) 
    params = jnp.hstack((qq, taus_deltas))
...
def test_make_En_and_grad():
    ...
    key = jax.random.PRNGKey(42)
    taus = jnp.array([0.5, 0.5, 0.5, 0.5])
