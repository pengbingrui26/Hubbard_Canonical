...
class Hubbard_1d(object):
    ...
    def nspin_up(self):
        spin_up = jnp.zeros((self.L*2, self.L*2))
        spin_up = spin_up.at[ :self.L, :self.L ].set(jnp.identity(self.L))
        return spin_up
            
    def nspin_down(self):
        spin_down = jnp.zeros((self.L*2, self.L*2))
        spin_down = spin_down.at[ self.L:, self.L: ].set(jnp.identity(self.L))
        return spin_down
